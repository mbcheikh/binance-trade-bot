...
class BinanceAPIManager:
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin):
        ...
        if target_coin.symbol != self.config.BRIDGE_SYMBOL:
            target_coin_price_bridge = self.get_ticker_price(target_symbol + self.config.BRIDGE_SYMBOL)
    ...
    def _sell_alt(self, origin_coin: Coin, target_coin: Coin):
        ...
        if self.config.MAX_AMOUNT:
            from_coin_price_bridge=from_coin_price
            if target_symbol != self.config.BRIDGE_SYMBOL:
                from_coin_price_bridge=self.get_ticker_price(origin_symbol+self.config.BRIDGE_SYMBOL)
            origin_balance_bridge=origin_balance*from_coin_price_bridge
            if origin_balance_bridge>self.config.MAX_AMOUNT + self.config.MIN_AMOUNT:
                origin_balance= self.config.MAX_AMOUNT/from_coin_price_bridge
